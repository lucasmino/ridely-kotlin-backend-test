...
class NearbyDriverAssemblerService(
    ...
    fun retrieveNearbyDrivers(driversLocationList: List<Long>): List<NearbyDriverDto> {
        log.info("Retrieving nearby drivers for ${driversLocationList.size} locations")
        return driversLocationList.map { driverLocation ->
            val driverData = driverRepo.findById(driverLocation).orElseThrow {
                val message = "Driver with ID ${driverLocation} not found"
                log.warn(message)
